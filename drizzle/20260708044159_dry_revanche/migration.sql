CREATE TYPE "agent_run_status" AS ENUM('running', 'success', 'failed');--> statement-breakpoint
CREATE TYPE "integration_provider" AS ENUM('gmail', 'google_calendar');--> statement-breakpoint
CREATE TYPE "subscription_status" AS ENUM('none', 'active', 'canceled', 'past_due');--> statement-breakpoint
CREATE TYPE "task_priority" AS ENUM('low', 'medium', 'high');--> statement-breakpoint
CREATE TYPE "task_status" AS ENUM('pending', 'completed', 'cancelled');--> statement-breakpoint
CREATE TABLE "agent_runs" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid(),
	"user_id" uuid NOT NULL,
	"status" "agent_run_status" DEFAULT 'running'::"agent_run_status" NOT NULL,
	"summary" text,
	"actions_log" jsonb DEFAULT '[]' NOT NULL,
...
ALTER TABLE "integrations" ADD CONSTRAINT "integrations_user_id_users_id_fkey" FOREIGN KEY ("user_id") REFERENCES "users"("id") ON DELETE CASCADE;--> statement-breakpoint
